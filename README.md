# fukugyo — 副業・フリーランスの雑務を全部AIに任せるツール

> 「今月何時間働いた？」「請求書どう作る？」「振り込まれてない…」
> そういう面倒ごとを、AIに全部やってもらうためのツールです。
...
**Claude Code** は、Anthropic が作った AI のコーディングアシスタントです。
ターミナル（黒い画面）上で動いて、「このファイルを読んで」「請求書を作って」と日本語で話しかけると実行してくれます。

fukugyo は、その Claude Code に「こう動いてね」と教えるための **スキル（設定セット）** です。
Claude Code をインストール済みなら、fukugyo を追加するだけで副業管理ができるようになります。

---

## インストール手順

### ① Claude Code をインストールする（まだの人）
...
入力した情報は **あなたのパソコンの中だけに保存されます**。
インターネットに送信されることはありません（詳しくは後述の「安全性について」を参照）。
...
| 三菱UFJ | `https://entry11.bk.mufg.jp/ibg/dfw/APLIN/loginib/login?_TRANID=AA000_001` |
| 三井住友 | `https://direct.smbc.co.jp/aib/aibgsjsf/pdFnLogin.do` |
| みずほ | `https://web.ib.mizuhobank.co.jp/servlet/LOGBNK0000000B.do` |
| ゆうちょ | `https://account.jp-bank.japanpost.jp/LogIn.do` |
| 楽天銀行 | `https://login.rakuten-bank.co.jp/` |
| PayPay銀行 | `https://direct.paypay-bank.co.jp/` |
| 住信SBI | `https://www.netbk.co.jp/contents/lineup/sougou/` |
| auじぶん銀行 | `https://ib.jibunbank.co.jp/pc/antifraud/login.html` |
...
このツールを使うにあたって、一番心配になるのが「口座番号や住所が漏れないか」だと思います。
結論から言うと、**漏れません**。以下の理由があります。

**保存場所について**
入力した口座番号・住所・クライアント情報などは、すべてあなたのパソコン内の `.fukugyo/` というフォルダに保存されます。Anthropic（Claude の開発元）のサーバーにも、インターネット上のどこにも送られません。
...
A. ターミナルはMacなら「アプリケーション」→「ユーティリティ」→「ターミナル」で開けます。Windowsなら「PowerShell」です。`python3 ...` のような文字を入力してEnterを押す操作を「コマンドを実行する」と言います。
...
A. 漏れません。`.fukugyo/config.json` に保存され、あなたのパソコンの外には出ません。GitHubに公開しても自動的に除外されます。

**Q. Slackを使っていなくても使えますか？**

A. 使えます。その場合はChromeの閲覧履歴から作業時間を推定します。

**Q. Chrome を閉じないといけないのはなぜですか？**

A. Chromeが起動中は履歴ファイルがロックされていて読めないためです。実行前に閉じてください。
